...
public class JobScheduleHelper {
            ...
            public void run() {
                ...
                while (!scheduleThreadToStop) {
                    ...
                    try {
                        ...
                        preparedStatement = conn.prepareStatement("select * from sys_job_lock where lock_name = 'schedule_lock' for update");
                        ...
                        List<JobEntity> scheduleList = XxlJobAdminConfig.getAdminConfig().getJobDao().scheduleJobQuery(nowTime + PRE_READ_MS, preReadCount);
                        if (scheduleList != null && scheduleList.size() > 0) {
                            // 2、push time-ring
                            for (JobEntity jobInfo : scheduleList) {
                            ...
                            for (JobEntity jobInfo : scheduleList) {
                                XxlJobAdminConfig.getAdminConfig().getJobDao().updateById(jobInfo);
                            }
    ...
    private void refreshNextValidTime(JobEntity jobInfo, Date fromTime) throws Exception {
    ...
    // ---------------------- tools ----------------------
    public static Date generateNextValidTime(JobEntity jobInfo, Date fromTime) throws Exception {
